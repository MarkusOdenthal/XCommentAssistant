...
def get_user_replies(client: tweepy.Client, user_id: int) -> List[tweepy.Tweet]:
    ...
    try:
        while True:
            ...
            if user_tweets.data:
                replies = [tweet for tweet in user_tweets.data if tweet.referenced_tweets and tweet.referenced_tweets[0].type == "replied_to"]
...
def get_original_posts(client: tweepy.Client, replies: List[tweepy.Tweet]) -> Dict[int, tweepy.Tweet]:
...
def filter_level1_interactions(replies: List[tweepy.Tweet], original_posts: Dict[int, tweepy.Tweet], user_id: int) -> List[Dict]:
    level1_interactions = []
